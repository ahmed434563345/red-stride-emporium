...
const MenJeans = () => {
  ...
  useEffect(() => {
    ...
    if (selectedColors.length > 0) {
      results = results.filter(p => p.colors && p.colors.some((c: string) => selectedColors.includes(c)));
    }
    if (selectedSizes.length > 0) {
      results = results.filter(p => p.sizes && p.sizes.some((s: string) => selectedSizes.includes(s)));
    }
    if (inStock) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">Men's Jeans</h1>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            ...
            {isLoading ? (
              ...
              <div className="text-center text-muted-foreground">No jeans found.</div>
            ) : (
